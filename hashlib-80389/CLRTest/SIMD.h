...
namespace SIMD
{
    ref class SIMDBase abstract
    {
        ...
        protected: void FFT_8(array<int>^ y, int a_index, int stripe)
        {
            int u;
            int v;

            u= y[a_index+stripe*0]; v= y[a_index+stripe*4]; y[a_index+stripe*0] = u+v; y[a_index+stripe*4] = (u-v) << (2*0);;
            u= y[a_index+stripe*1]; v= y[a_index+stripe*5]; y[a_index+stripe*1] = u+v; y[a_index+stripe*5] = (u-v) << (2*1);;
            u= y[a_index+stripe*2]; v= y[a_index+stripe*6]; y[a_index+stripe*2] = u+v; y[a_index+stripe*6] = (u-v) << (2*2);;
            u= y[a_index+stripe*3]; v= y[a_index+stripe*7]; y[a_index+stripe*3] = u+v; y[a_index+stripe*7] = (u-v) << (2*3);;

            y[a_index+stripe*6] = ((y[a_index+stripe*6] & 255) - (y[a_index+stripe*6] >> 8));
            y[a_index+stripe*7] = ((y[a_index+stripe*7] & 255) - (y[a_index+stripe*7] >> 8));

            u= y[a_index+stripe*0]; v= y[a_index+stripe*2]; y[a_index+stripe*0] = u+v; y[a_index+stripe*2] = (u-v) << (2*0);;
            u= y[a_index+stripe*4]; v= y[a_index+stripe*6]; y[a_index+stripe*4] = u+v; y[a_index+stripe*6] = (u-v) << (2*0);;
            u= y[a_index+stripe*1]; v= y[a_index+stripe*3]; y[a_index+stripe*1] = u+v; y[a_index+stripe*3] = (u-v) << (2*2);;
            u= y[a_index+stripe*5]; v= y[a_index+stripe*7]; y[a_index+stripe*5] = u+v; y[a_index+stripe*7] = (u-v) << (2*2);;

            y[a_index+stripe*7] = ((y[a_index+stripe*7] & 255) - (y[a_index+stripe*7] >> 8));

            u= y[a_index+stripe*0]; v= y[a_index+stripe*1]; y[a_index+stripe*0] = u+v; y[a_index+stripe*1] = (u-v) << (2*0);;
            u= y[a_index+stripe*2]; v= y[a_index+stripe*3]; y[a_index+stripe*2] = u+v; y[a_index+stripe*3] = (u-v) << (2*0);;
            u= y[a_index+stripe*4]; v= y[a_index+stripe*5]; y[a_index+stripe*4] = u+v; y[a_index+stripe*5] = (u-v) << (2*0);;
            u= y[a_index+stripe*6]; v= y[a_index+stripe*7]; y[a_index+stripe*6] = u+v; y[a_index+stripe*7] = (u-v) << (2*0);;

            y[a_index+stripe*0] = ((y[a_index+stripe*0] & 255) - (y[a_index+stripe*0] >> 8)); y[a_index+stripe*0] = (y[a_index+stripe*0] <= 128 ? y[a_index+stripe*0] : y[a_index+stripe*0]-257);;
            y[a_index+stripe*1] = ((y[a_index+stripe*1] & 255) - (y[a_index+stripe*1] >> 8)); y[a_index+stripe*1] = (y[a_index+stripe*1] <= 128 ? y[a_index+stripe*1] : y[a_index+stripe*1]-257);;
            y[a_index+stripe*2] = ((y[a_index+stripe*2] & 255) - (y[a_index+stripe*2] >> 8)); y[a_index+stripe*2] = (y[a_index+stripe*2] <= 128 ? y[a_index+stripe*2] : y[a_index+stripe*2]-257);;
            y[a_index+stripe*3] = ((y[a_index+stripe*3] & 255) - (y[a_index+stripe*3] >> 8)); y[a_index+stripe*3] = (y[a_index+stripe*3] <= 128 ? y[a_index+stripe*3] : y[a_index+stripe*3]-257);;
            y[a_index+stripe*4] = ((y[a_index+stripe*4] & 255) - (y[a_index+stripe*4] >> 8)); y[a_index+stripe*4] = (y[a_index+stripe*4] <= 128 ? y[a_index+stripe*4] : y[a_index+stripe*4]-257);;
            y[a_index+stripe*5] = ((y[a_index+stripe*5] & 255) - (y[a_index+stripe*5] >> 8)); y[a_index+stripe*5] = (y[a_index+stripe*5] <= 128 ? y[a_index+stripe*5] : y[a_index+stripe*5]-257);;
            y[a_index+stripe*6] = ((y[a_index+stripe*6] & 255) - (y[a_index+stripe*6] >> 8)); y[a_index+stripe*6] = (y[a_index+stripe*6] <= 128 ? y[a_index+stripe*6] : y[a_index+stripe*6]-257);;
            y[a_index+stripe*7] = ((y[a_index+stripe*7] & 255) - (y[a_index+stripe*7] >> 8)); y[a_index+stripe*7] = (y[a_index+stripe*7] <= 128 ? y[a_index+stripe*7] : y[a_index+stripe*7]-257);;
        }

        protected: virtual void TransformBlock(array<byte>^ a_data, int a_index, int a_final) = 0;

        protected: virtual void TransformBlock(array<byte>^ a_data, int a_index)
        {
            TransformBlock(a_data, a_index, 0);
        }

        protected: array<byte>^ SIMDBase::GetResult()
        {
        ...
        protected: virtual void Finish()
        {
            ...
            array<byte>^ pad = gcnew array<byte>(BlockSize);
            ...
            TransformBlock(pad, 0, (m_processedBytes  < 2048) ? 2 : 1);
        }
        ...
        public: SIMDBase(int a_hashSize, int a_blockSize)
        {
            ...
            m_state = gcnew array<uint>(BlockSize / 4);
        }
    ...
    ref class SIMD256Base : SIMDBase
    {
        #pragma region Consts

        private: static array<uint>^ IV_224 = 
        {
        ...
        private: static array<uint>^ IV_256 = 
        {
        ...
        private: static array<int>^ FFT64_8_8_Twiddle = 
        {
        ...
        private: static array<int>^ FFT128_2_64_Twiddle =  
        {
        ...
        private: void Round4(array<int>^ y, int i, int r, int s, int t, int u)
        {
            int code = i<2 ? 185 : 233;
            array<uint>^ w = gcnew array<uint>(32);
            int i32 = i * 32;

            for (int a=0; a<32; a++)
                w[a] = (((uint) (y[Q4[i32+a]] * code)) << 16) | (((uint) (y[P4[i32+a]] * code)) & 0xffff);

            array<uint>^ R = gcnew array<uint>(4);
        ...
        private: void FFT_64(array<int>^ y, int a_index)
        {
            for (int i=0; i<8; i++) 
                FFT_8(y,a_index+i,8);
            ...
            {
                if (i & 7) 
                    y[a_index+i] = ((y[a_index+i]*FFT64_8_8_Twiddle[i] & 255) - (y[a_index+i]*FFT64_8_8_Twiddle[i] >> 8));
            }

            for (int i=0; i<8; i++) 
                FFT_8(y,a_index+8*i,1);
        }

        protected: virtual void TransformBlock(array<byte>^ a_data, int a_index, int a_final) override
        {
            array<int>^ y = gcnew array<int>(128);
            ...
            array<uint>^ state = gcnew array<uint>(16);
            Array::Copy(m_state, 0, state, 0, m_state->Length);

            array<uint>^ message = Converters::ConvertBytesToUInts(a_data, a_index, 64);
    ...
    ref class SIMD512Base : SIMDBase
    {
        #pragma region Consts

        private: static array<uint>^ IV_384 = 
        {
        ...
        private: static array<uint>^ IV_512 = 
        {
        ...
        private: static array<int>^ FFT128_8_16_Twiddle =  
        {
        ...
        private: static array<int>^ FFT256_2_128_Twiddle =  
        {
        ...
        private: static array<int>^ p8_xor = 
        {
            1, 6, 2, 3, 5, 7, 4
        };
        ...
        private: void Round8(array<int>^ y, int i, int r, int s, int t, int u)
        {
            int code = i<2 ? 185 : 233;
            array<uint>^ w = gcnew array<uint>(64);

            for(int a=0; a<64; a++)
                w[a] = (((uint) (y[Q8[64*i+a]] * code)) << 16) | (((uint) (y[P8[64*i+a]] * code)) & 0xffff);

            array<uint>^ R = gcnew array<uint>(8);
        ...
        private: void FFT_16(array<int>^ y, int a_index, int stripe)
        {
            int u;
            int v;

            u= y[a_index+stripe*0]; v= y[a_index+stripe*8]; y[a_index+stripe*0] = u+v; y[a_index+stripe*8] = (u-v) << 0;;
            u= y[a_index+stripe*1]; v= y[a_index+stripe*9]; y[a_index+stripe*1] = u+v; y[a_index+stripe*9] = (u-v) << 1;;
            u= y[a_index+stripe*2]; v= y[a_index+stripe*10]; y[a_index+stripe*2] = u+v; y[a_index+stripe*10] = (u-v) << 2;;
            u= y[a_index+stripe*3]; v= y[a_index+stripe*11]; y[a_index+stripe*3] = u+v; y[a_index+stripe*11] = (u-v) << 3;;
            u= y[a_index+stripe*4]; v= y[a_index+stripe*12]; y[a_index+stripe*4] = u+v; y[a_index+stripe*12] = (u-v) << 4;;
            u= y[a_index+stripe*5]; v= y[a_index+stripe*13]; y[a_index+stripe*5] = u+v; y[a_index+stripe*13] = (u-v) << 5;;
            u= y[a_index+stripe*6]; v= y[a_index+stripe*14]; y[a_index+stripe*6] = u+v; y[a_index+stripe*14] = (u-v) << 6;;
            u= y[a_index+stripe*7]; v= y[a_index+stripe*15]; y[a_index+stripe*7] = u+v; y[a_index+stripe*15] = (u-v) << 7;;

            y[a_index+stripe*11] = ((y[a_index+stripe*11] & 255) - (y[a_index+stripe*11] >> 8));
            y[a_index+stripe*12] = ((y[a_index+stripe*12] & 255) - (y[a_index+stripe*12] >> 8));
            y[a_index+stripe*13] = ((y[a_index+stripe*13] & 255) - (y[a_index+stripe*13] >> 8));
            y[a_index+stripe*14] = ((y[a_index+stripe*14] & 255) - (y[a_index+stripe*14] >> 8));
            y[a_index+stripe*15] = ((y[a_index+stripe*15] & 255) - (y[a_index+stripe*15] >> 8));

            u= y[a_index+stripe*0]; v= y[a_index+stripe*4]; y[a_index+stripe*0] = u+v; y[a_index+stripe*4] = (u-v) << 0;;
            u= y[a_index+stripe*8]; v= y[a_index+stripe*12]; y[a_index+stripe*8] = u+v; y[a_index+stripe*12] = (u-v) << 0;;
            u= y[a_index+stripe*1]; v= y[a_index+stripe*5]; y[a_index+stripe*1] = u+v; y[a_index+stripe*5] = (u-v) << 2;;
            u= y[a_index+stripe*9]; v= y[a_index+stripe*13]; y[a_index+stripe*9] = u+v; y[a_index+stripe*13] = (u-v) << 2;;
            u= y[a_index+stripe*2]; v= y[a_index+stripe*6]; y[a_index+stripe*2] = u+v; y[a_index+stripe*6] = (u-v) << 4;;
            u= y[a_index+stripe*10]; v= y[a_index+stripe*14]; y[a_index+stripe*10] = u+v; y[a_index+stripe*14] = (u-v) << 4;;
            u= y[a_index+stripe*3]; v= y[a_index+stripe*7]; y[a_index+stripe*3] = u+v; y[a_index+stripe*7] = (u-v) << 6;;
            u= y[a_index+stripe*11]; v= y[a_index+stripe*15]; y[a_index+stripe*11] = u+v; y[a_index+stripe*15] = (u-v) << 6;;

            y[a_index+stripe*5] = ((y[a_index+stripe*5] & 255) - (y[a_index+stripe*5] >> 8));
            y[a_index+stripe*7] = ((y[a_index+stripe*7] & 255) - (y[a_index+stripe*7] >> 8));
            y[a_index+stripe*13] = ((y[a_index+stripe*13] & 255) - (y[a_index+stripe*13] >> 8));
            y[a_index+stripe*15] = ((y[a_index+stripe*15] & 255) - (y[a_index+stripe*15] >> 8));

            u= y[a_index+stripe*0]; v= y[a_index+stripe*2]; y[a_index+stripe*0] = u+v; y[a_index+stripe*2] = (u-v) << 0;;
            u= y[a_index+stripe*4]; v= y[a_index+stripe*6]; y[a_index+stripe*4] = u+v; y[a_index+stripe*6] = (u-v) << 0;;
            u= y[a_index+stripe*8]; v= y[a_index+stripe*10]; y[a_index+stripe*8] = u+v; y[a_index+stripe*10] = (u-v) << 0;;
            u= y[a_index+stripe*12]; v= y[a_index+stripe*14]; y[a_index+stripe*12] = u+v; y[a_index+stripe*14] = (u-v) << 0;;
            u= y[a_index+stripe*1]; v= y[a_index+stripe*3]; y[a_index+stripe*1] = u+v; y[a_index+stripe*3] = (u-v) << 4;;
            u= y[a_index+stripe*5]; v= y[a_index+stripe*7]; y[a_index+stripe*5] = u+v; y[a_index+stripe*7] = (u-v) << 4;;
            u= y[a_index+stripe*9]; v= y[a_index+stripe*11]; y[a_index+stripe*9] = u+v; y[a_index+stripe*11] = (u-v) << 4;;
            u= y[a_index+stripe*13]; v= y[a_index+stripe*15]; y[a_index+stripe*13] = u+v; y[a_index+stripe*15] = (u-v) << 4;;

            u= y[a_index+stripe*0]; v= y[a_index+stripe*1]; y[a_index+stripe*0] = u+v; y[a_index+stripe*1] = (u-v) << 0;;
            u= y[a_index+stripe*2]; v= y[a_index+stripe*3]; y[a_index+stripe*2] = u+v; y[a_index+stripe*3] = (u-v) << 0;;
            u= y[a_index+stripe*4]; v= y[a_index+stripe*5]; y[a_index+stripe*4] = u+v; y[a_index+stripe*5] = (u-v) << 0;;
            u= y[a_index+stripe*6]; v= y[a_index+stripe*7]; y[a_index+stripe*6] = u+v; y[a_index+stripe*7] = (u-v) << 0;;
            u= y[a_index+stripe*8]; v= y[a_index+stripe*9]; y[a_index+stripe*8] = u+v; y[a_index+stripe*9] = (u-v) << 0;;
            u= y[a_index+stripe*10]; v= y[a_index+stripe*11]; y[a_index+stripe*10] = u+v; y[a_index+stripe*11] = (u-v) << 0;;
            u= y[a_index+stripe*12]; v= y[a_index+stripe*13]; y[a_index+stripe*12] = u+v; y[a_index+stripe*13] = (u-v) << 0;;
            u= y[a_index+stripe*14]; v= y[a_index+stripe*15]; y[a_index+stripe*14] = u+v; y[a_index+stripe*15] = (u-v) << 0;;
            ...
            y[a_index+stripe*6] = ((y[a_index+stripe*6] & 255) - (y[a_index+stripe*6] >> 8)); y[a_index+stripe*6] = (y[a_index+stripe*6] <= 128 ? y[a_index+stripe*6] : y[a_index+stripe*6]-257);;
            y[a_index+stripe*7] = ((y[a_index+stripe*7] & 255) - (y[a_index+stripe*7] >> 8)); y[a_index+stripe*7] = (y[a_index+stripe*7] <= 128 ? y[a_index+stripe*7] : y[a_index+stripe*7]-257);;
            y[a_index+stripe*8] = ((y[a_index+stripe*8] & 255) - (y[a_index+stripe*8] >> 8)); y[a_index+stripe*8] = (y[a_index+stripe*8] <= 128 ? y[a_index+stripe*8] : y[a_index+stripe*8]-257);;
            y[a_index+stripe*9] = ((y[a_index+stripe*9] & 255) - (y[a_index+stripe*9] >> 8)); y[a_index+stripe*9] = (y[a_index+stripe*9] <= 128 ? y[a_index+stripe*9] : y[a_index+stripe*9]-257);;
            y[a_index+stripe*10] = ((y[a_index+stripe*10] & 255) - (y[a_index+stripe*10] >> 8)); y[a_index+stripe*10] = (y[a_index+stripe*10] <= 128 ? y[a_index+stripe*10] : y[a_index+stripe*10]-257);;
            y[a_index+stripe*11] = ((y[a_index+stripe*11] & 255) - (y[a_index+stripe*11] >> 8)); y[a_index+stripe*11] = (y[a_index+stripe*11] <= 128 ? y[a_index+stripe*11] : y[a_index+stripe*11]-257);;
            y[a_index+stripe*12] = ((y[a_index+stripe*12] & 255) - (y[a_index+stripe*12] >> 8)); y[a_index+stripe*12] = (y[a_index+stripe*12] <= 128 ? y[a_index+stripe*12] : y[a_index+stripe*12]-257);;
            y[a_index+stripe*13] = ((y[a_index+stripe*13] & 255) - (y[a_index+stripe*13] >> 8)); y[a_index+stripe*13] = (y[a_index+stripe*13] <= 128 ? y[a_index+stripe*13] : y[a_index+stripe*13]-257);;
            y[a_index+stripe*14] = ((y[a_index+stripe*14] & 255) - (y[a_index+stripe*14] >> 8)); y[a_index+stripe*14] = (y[a_index+stripe*14] <= 128 ? y[a_index+stripe*14] : y[a_index+stripe*14]-257);;
            y[a_index+stripe*15] = ((y[a_index+stripe*15] & 255) - (y[a_index+stripe*15] >> 8)); y[a_index+stripe*15] = (y[a_index+stripe*15] <= 128 ? y[a_index+stripe*15] : y[a_index+stripe*15]-257);;
        }

        private: void FFT_128_full(array<int>^ y, int a_index)
        {
            for (int i=0; i<16; i++) 
                FFT_8(y,a_index+i,16);

            for (int i=0; i<128; i++)
                y[a_index+i] = ((y[a_index+i]*FFT128_8_16_Twiddle[i] & 255) - (y[a_index+i]*FFT128_8_16_Twiddle[i] >> 8));

            for (int i=0; i<8; i++) 
                FFT_16(y, a_index+16*i,1);
        }

        protected: virtual void TransformBlock(array<byte>^ a_data, int a_index, int a_final) override
        {
            array<int>^ y = gcnew array<int>(256);
            ...
            array<uint>^ state = gcnew array<uint>(32);
            Array::Copy(m_state, 0, state, 0, 32);

            array<uint>^ message = Converters::ConvertBytesToUInts(a_data, a_index, 128);
