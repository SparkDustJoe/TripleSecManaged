...
namespace Echo
{
    ref class EchoBase abstract
    {
        protected:

            #pragma region Consts

            static array<uint>^ s_T0 = 
            {
            ...
            Trans::HashBuffer^ m_buffer;
            int m_blockSize;

            array<byte>^ GetResult()
            {
                array<byte>^ result = gcnew array<byte>(HashSize);
                array<byte>^ state = Converters::ConvertULongsToBytes(m_state, 0, HashSize / 8 + ((HashSize % 8 != 0) ? 1 : 0));
                Array::Copy(state, result, m_hashSize);
                return result;
            }
            ...
            void Finish()
            {
                Array::Clear(m_pad, 0, m_pad->Length);
                m_pad[0] = 0x80;
                ...
                {
                    int pad_len = BlockSize - m_buffer->Pos;
                    m_processedBytes -= pad_len;
                    TransformBytes(m_pad, 0, BlockSize - m_buffer->Pos);
                    m_processedBytes += pad_len;

                    Array::Clear(m_pad, 0, m_pad->Length);
                } 

                int index = BlockSize - m_buffer->Pos - 18;

                m_pad[index++] = (byte)(m_hashSize * 8);
                m_pad[index++] = (byte)((m_hashSize * 8) >> 8);

                m_pad[index++] = (byte)(bits >> 0);
                m_pad[index++] = (byte)(bits >> 8);
                m_pad[index++] = (byte)(bits >> 16);
                m_pad[index++] = (byte)(bits >> 24);
                m_pad[index++] = (byte)(bits >> 32);
                m_pad[index++] = (byte)(bits >> 40);
                m_pad[index++] = (byte)(bits >> 48);
                m_pad[index] = (byte)(bits >> 56);
        ...
        public:
            ...
            EchoBase(int a_hashSize, int a_blockSize)
            {
                m_hashSize = a_hashSize;
                m_blockSize = a_blockSize;

                m_state = gcnew array<ulong>(32);
                m_cv = gcnew array<ulong>(16);

                m_pad = gcnew array<byte>(BlockSize);
    ...
    ref class Echo256Base : public EchoBase
    {
        protected:

            virtual void TransformBlock(array<byte>^ a_data, int a_index) override
            {
                Buffer::BlockCopy(a_data, a_index, m_state, 8*8, BlockSize);
                ...
                for (int r=1; r<4; r++)
                {
                    m_cv[0] ^= m_state[8*r+0]; 
                    m_cv[1] ^= m_state[8*r+1];
                    m_cv[2] ^= m_state[8*r+2];
                    m_cv[3] ^= m_state[8*r+3]; 
                    m_cv[4] ^= m_state[8*r+4]; 
                    m_cv[5] ^= m_state[8*r+5]; 
                    m_cv[6] ^= m_state[8*r+6]; 
                    m_cv[7] ^= m_state[8*r+7]; 
                }
                ...
                WL11 ^= WL15 ^ WL7  ^ WL3; 
                WH11 ^= WH15 ^ WH7  ^ WH3; 
                m_state[0] = m_cv[0] ^ WL8; 
                m_state[1] = m_cv[1] ^ WH8; 
                m_state[2] = m_cv[2] ^ WL9; 
                m_state[3] = m_cv[3] ^ WH9; 
                m_state[4] = m_cv[4] ^ WL10; 
                m_state[5] = m_cv[5] ^ WH10; 
                m_state[6] = m_cv[6] ^ WL11; 
                m_state[7] = m_cv[7] ^ WH11; 
            }

        public:

            Echo256Base(int a_hashSize) 
                : EchoBase(a_hashSize, 192)
            {
                Initialize();
            }
            ...
            virtual void Initialize() override
            {
                ...
                for (int i=0; i<4; i++)
                    m_state[2*i] = m_hashSize * 8;
            }
    };

    ref class Echo512Base : public EchoBase
    {
        protected:

            virtual void TransformBlock(array<byte>^ a_data, int a_index) override
            {
                Buffer::BlockCopy(a_data, a_index, m_state, 16*8, BlockSize);
        
                uint WA, WB, WC, WD;

                m_cv[0] = m_state[0] ^ m_state[16]; 
                m_cv[1] = m_state[1] ^ m_state[17];
                m_cv[2] = m_state[2] ^ m_state[18];
                m_cv[3] = m_state[3] ^ m_state[19];  
                m_cv[4] = m_state[4] ^ m_state[20];  
                m_cv[5] = m_state[5] ^ m_state[21];  
                m_cv[6] = m_state[6] ^ m_state[22];  
                m_cv[7] = m_state[7] ^ m_state[23];  
                m_cv[8] = m_state[8] ^ m_state[24];  
                m_cv[9] = m_state[9] ^ m_state[25]; 
                m_cv[10] = m_state[10] ^ m_state[26]; 
                m_cv[11] = m_state[11] ^ m_state[27];  
                m_cv[12] = m_state[12] ^ m_state[28];  
                m_cv[13] = m_state[13] ^ m_state[29];  
                m_cv[14] = m_state[14] ^ m_state[30];  
                m_cv[15] = m_state[15] ^ m_state[31];  
                ...
                m_state[0] = m_cv[0] ^ WL0 ^ WL8; 
                m_state[1] = m_cv[1] ^ WH0 ^ WH8; 
                m_state[2] = m_cv[2] ^ WL1 ^ WL9; 
                m_state[3] = m_cv[3] ^ WH1 ^ WH9; 
                m_state[4] = m_cv[4] ^ WL2 ^ WL10; 
                m_state[5] = m_cv[5] ^ WH2 ^ WH10; 
                m_state[6] = m_cv[6] ^ WL3 ^ WL11; 
                m_state[7] = m_cv[7] ^ WH3 ^ WH11; 
                m_state[8] = m_cv[8] ^ WL4 ^ WL12; 
                m_state[9] = m_cv[9] ^ WH4 ^ WH12; 
                m_state[10] = m_cv[10] ^ WL5 ^ WL13; 
                m_state[11] = m_cv[11] ^ WH5 ^ WH13; 
                m_state[12] = m_cv[12] ^ WL6 ^ WL14; 
                m_state[13] = m_cv[13] ^ WH6 ^ WH14; 
                m_state[14] = m_cv[14] ^ WL7 ^ WL15; 
                m_state[15] = m_cv[15] ^ WH7 ^ WH15; 
            }

        public:

            Echo512Base(int a_hashSize) 
                : EchoBase(a_hashSize, 128)
            {
                Initialize();
            }
