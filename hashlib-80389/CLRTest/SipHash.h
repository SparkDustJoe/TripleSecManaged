...
namespace SipHash
{
    ref class SipHash
    {
        private:
            ...
            ulong m_key0;
            ulong m_key1;
            array<byte>^ m_buffer = gcnew array<byte>(8);
            ulong m_processed_bytes;

            void TransformBlock(array<byte>^ a_data, int a_index)
            {
                ulong m = HashLib::Converters::ConvertBytesToULong(a_data, a_index);
            ...
            array<byte>^ GetResult()
            {
                ...
                array<byte>^ result = gcnew array<byte>(8);
        ...
        public:
            ...
            SipHash()
            {
                array<byte>^ key = gcnew array<byte>(16);
                for (int j = 0; j < 16; j++)
                    key[j] = j;
                SetKey(key);
            ...
            void TransformBytes(array<byte>^ a_data, int a_index, int a_length)
            {
                ...
                Trans::memset(m_buffer, 0, BlockSize);
