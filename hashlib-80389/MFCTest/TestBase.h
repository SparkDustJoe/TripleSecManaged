        ...
        public: static void Init()
        {
            ...
            if(GetProcessAffinityMask(ph, &afp, &afs))
            {
                ...
                if (!SetProcessAffinityMask(ph, afp))
                    wprintf(L"ERROR: Couldn't set Process Affinity Mask\n\n"); 
            }
            else
                wprintf(L"ERROR: Couldn't get Process Affinity Mask\n\n"); 

            if (!SetPriorityClass(GetCurrentProcess(), ABOVE_NORMAL_PRIORITY_CLASS))
                wprintf(L"ERROR: Couldn't set process priority\n\n");

            if (!SetThreadPriority(GetCurrentThread(), THREAD_PRIORITY_ABOVE_NORMAL))
                wprintf(L"ERROR: Couldn't set thread priority\n\n");

            #endif
        }

        public: virtual byte* ComputeBytes(byte* a_data, int a_length) = 0;
        protected: virtual CString GetTestVectorsDir() = 0;
        protected: virtual CString GetTestName() = 0;
        protected: virtual int GetMaxBufferSize() = 0;
