...
namespace Fugue
{
    ...
    const ulong aes_style_table0[256] = 
    {
    ...
    const ulong aes_style_table1[256] = 
    {
    ...
    const ulong aes_style_table2[256] = 
    {
    ...
    const ulong aes_style_table3[256] = 
    {
    ...
    void FugueBase::Initialize()
    {
        m_base = 0;
        memset(m_buffer, 0, 4);
        m_processedBytes = 0;
        memset(m_state, 0, BlockSize*4-HashSize);
    }

    void FugueBase::TransformBytes(byte* a_data, int a_index, int a_length)
    {
        a_data += a_index;
        ...
        if (need < 4)
        {
            if (need > a_length)
            {
                memcpy(&m_buffer[4-need], a_data, a_length);
                m_processedBytes+= a_length;
                return;
            }
            else
            {
                memcpy(&m_buffer[4-need], a_data, need);

                TransformBytes_(&m_buffer[0], 0, 4);

                m_processedBytes+= need;
                a_length -= need;
                a_data+= need;
            }
        }
        ...
        if (a_length >= 4)
        {
            TransformBytes_(a_data, 0, a_length);

            m_processedBytes += (a_length/4)*4;
            a_data += (a_length/4)*4;
            a_length &= 3;
        }
        ...
        if (a_length != 0)
        {
            memcpy(&m_buffer[0], a_data, a_length);
            m_processedBytes += a_length;
        }
    }

    void FugueBase::Finish()
    {
        int need = 4-(m_processedBytes%4);

        if (need < 4)
        {
            memset(&m_buffer[4-need], 0, need);
            TransformBytes_(&m_buffer[0], 0, 4);
        }
        ...
        TransformBytes_((byte*)&bits, 0, 8);
    }

    byte* FugueBase::TransformFinal()
    {
        Finish();
    ...
    byte* FugueBase::ComputeBytes(byte* a_data, int a_length)
    {
        Initialize();
