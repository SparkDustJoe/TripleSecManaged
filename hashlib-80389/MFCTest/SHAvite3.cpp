...
#pragma region Consts
    ...
    uint IV_224[8] =  {0xC4C67795, 0xC0B1817F, 0xEAD88924, 0x1ABB1BB0,
        0xE0C29152, 0xBDE046BA, 0xAEEECF99, 0x58D509D8};

    uint IV_256[8] =  {0x3EECF551, 0xBF10819B, 0xE6DC8559, 0xF3E23FD5,
        0x431AEC73, 0x79E3F731, 0x98325F05, 0xA92A31F1};

    uint IV_384[16] = {0x71F48510, 0xA903A8AC, 0xFE3216DD, 0x0B2D2AD4,
    ...
    uint IV_512[16] = {0xD5652B63, 0x25F1E6EA, 0xB18F48FA, 0xA1EE3A47,
...
#pragma endregion
    ...
    void SHAvite3_256Base::TransformBlock(byte* a_data, int a_index)
    {
        uint x0,x1,x2,x3,y0,y1,y2,y3;

        uint* rk = new uint[144];
        memcpy(rk, &a_data[a_index], 64);

        uint cnt0 = (uint)(m_processedBytes << 3);
        uint cnt1 = (uint)(m_processedBytes >> 29);
    ...
    void SHAvite3_512Base::TransformBlock(byte* a_data, int a_index)
    {
        uint x0,x1,x2,x3,y0,y1,y2,y3;

        uint* rk = new uint[448];
        memcpy(rk, &a_data[a_index], 128);
    ...
    void SHAvite3_256Base::Initialize()
    {
        if (HashSize == 28)
            memcpy(m_state, IV_224, 32);
        else
            memcpy(m_state, IV_256, 32);

        SHAvite3Base::Initialize();
    }

    void SHAvite3_512Base::Initialize()
    {
        if (HashSize == 48)
            memcpy(m_state, IV_384, 64);
        else
            memcpy(m_state, IV_512, 64);

        SHAvite3Base::Initialize();
    }

    void SHAvite3Base::TransformBytes(byte* a_data, int a_index, int a_length)
    {
        if (m_buffer_pos > 0)
        {
        ...
        if (a_length > 0)
        {
            memcpy(m_buffer, &a_data[a_index], a_length);
            m_processedBytes += a_length;
            m_buffer_pos += a_length;
        }
    }

    void SHAvite3_512Base::Finish()
    {
        byte pad[128];

        memset(pad, 0, BlockSize);
        memcpy(pad, m_buffer, m_buffer_pos);

        pad[m_buffer_pos] = 0x80;

        if (m_buffer_pos>=BlockSize-18)
        {
    ...
    SHAvite3Base::SHAvite3Base(int a_hashSize, int a_blockSize)
    {
        HashSize = a_hashSize;
    ...
    SHAvite3Base::~SHAvite3Base()
    {
        delete m_state;
        delete m_buffer;
    }

    byte* SHAvite3Base::GetResult()
    {
    ...
    byte* SHAvite3Base::TransformFinal()
    {
        Finish();
    ...
    byte* SHAvite3Base::ComputeBytes(byte* a_data, int a_length)
    {
        Initialize();
