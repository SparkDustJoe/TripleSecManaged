...
#define RND1024Q(x,y,r) do {					\
    ...
    HashReturn Init(hashState* ctx,
        ...
        if (hashbitlen <= 256) {
            ctx->size = SHORT;
            ctx->chaining = new u64[COLS512];
            ctx->buffer = new BitSequence[SIZE512];
            memset(ctx->chaining, 0, COLS512 * sizeof(u64));
            memset(ctx->buffer, 0, SIZE512 * sizeof(BitSequence));
            /* set initial value */
            ctx->chaining[COLS512 - 1] = U64BIG((u64)hashbitlen);
        }
        else {
            ctx->size = LONG;
            ctx->chaining = new u64[COLS1024];
            ctx->buffer = new BitSequence[SIZE1024];
            memset(ctx->chaining, 0, COLS1024 * sizeof(u64));
            memset(ctx->buffer, 0, SIZE1024 * sizeof(BitSequence));
            /* set initial value */
