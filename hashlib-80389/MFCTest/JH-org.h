...
#define SS(m0,m1,m2,m3,m4,m5,m6,m7,cc0,cc1)   \
    ...
    HashReturn Update(hashState *state, const BitSequence *data, DataLength databitlen)
    {
        ...
        if ((state->datasize_in_buffer > 0) && ((state->datasize_in_buffer + databitlen) < 512))
        {
            if ((databitlen & 7) == 0)
            {
                memcpy(state->buffer + (state->datasize_in_buffer >> 3), data, 64 - (int)(state->datasize_in_buffer >> 3));
            }
            else memcpy(state->buffer + (state->datasize_in_buffer >> 3), data, 64 - (int)(state->datasize_in_buffer >> 3) + 1);
